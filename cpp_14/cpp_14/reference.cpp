...
int main() {
    int i;
    f(i); // argument is lvalue, calls f<int&>(int&), std::forward<T>(x) is lvalue
    f(0); // argument is rvalue: calls f<int>(int&&), std::forward<T>(x) is rvalue
}

template<class T>
int g(const T&& y); // y is not a forwarding reference: const T is not exactly T
